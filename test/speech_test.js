...
describe("Speech", function () {
  ...
  describe("basics", function () {
    ...
    describe("say", function () {
      ...
      it("should generate a saying tag with a voice", function () {
        speech.say("hi", {
          name: "Brian",
        });
        assert.equal(
          speech.ssml(),
          '<speak><voice name="Brian">hi</voice></speak>'
        );
      });
      it("should generate a saying tag with a lang", function () {
        speech.say("hi", {
          lang: "pt-BR",
        });
        assert.equal(
          speech.ssml(),
          '<speak><lang xml:lang="pt-BR">hi</lang></speak>'
        );
      });
      it("should generate a saying tag with a voice and lang", function () {
        speech.say("hi", {
          name: "Brian",
          lang: "en-US",
        });
        assert.equal(
          speech.ssml(),
          '<speak><voice name="Brian"><lang xml:lang="en-US">hi</lang></voice></speak>'
        );
      });
    });

    describe("emotion", function () {
      it("should generate a saying tag with an emotion and intensity", function () {
        speech.emotion("hi", {
          name: "excited",
          intensity: "medium",
        });
        assert.equal(
          speech.ssml(),
          '<speak><amazon:emotion name="excited" intensity="medium">hi</amazon:emotion></speak>'
        );
      });
    });
    ...
    describe("pauseByStrength", function () {
      describe("positive", function () {
        it("should generate the tag with valid strength", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><break strength='x-weak'/></speak>"
          );
        });
    ...
    describe("audio", function () {
      describe("positive", function () {
        it("should build a audio tag", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><audio src='http://www.audio.com/sound.mp3'/></speak>"
          );
        });
        ...
        it("should build a audio tag with nested SSML", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><audio src='http://www.audio.com/sound.mp3'>wow</audio></speak>"
          );
        });
      });
    ...
    describe("spell", function () {
      it("should build a spell tag", function () {
        ...
        assert.equal(
          speech.ssml(),
          "<speak><say-as interpret-as='spell-out'>mandy</say-as></speak>"
        );
      });
      ...
      it("spell should escape characters", function () {
        ...
        assert.equal(
          speech.ssml(),
          "<speak><say-as interpret-as='spell-out'>&lt;mandy&apos;s&gt;</say-as></speak>"
        );
      });
    });
    ...
    describe("sayWithSSML", function () {
      describe("positive", function () {
        it("should push the raw ssml string into the speech object", function () {
          ...
          assert.equal(
            speech.ssml(),
            '<speak>we should all <w role="amazon:VB">read</w> more</speak>'
          );
        });
      });
  ...
  describe("sayAs", function () {
    ...
    it("should build a sayAs interpret as cardinal number", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='cardinal'>five</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as unit", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='unit'>2N</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as telephone and format 39", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='telephone' format='39'>+39(011)777-7777</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as telephone and format 39", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='telephone' format='39'>+1-800-EXAMPLE</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as address and format us-state", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='address' format='us-state'>CO</say-as></speak>"
      );
    });
  ...
  describe("time", function () {
    ...
    it("should build a sayAs interpret as time", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='time'>2'10\"</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as time and format hms24", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='time' format='hms24'>19:21:30</say-as></speak>"
      );
    });
    ...
    it("should build a sayAs interpret as time and format hms12", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><say-as interpret-as='time' format='hms12'>09:21:15</say-as></speak>"
      );
    });
  });
  ...
  describe("part of speech", function () {
    ...
    it("should build a w which has a role of ivona:VBD", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><w role='ivona:VBD'>read</w></speak>"
      );
    });
    ...
    it("should build a w which has a role of ivona:NN", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><w role='ivona:NN'>conduct</w></speak>"
      );
    });
    ...
    it("should build a w which has a role of ivona:SENSE_1", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><w role='ivona:SENSE_1'>bass</w></speak>"
      );
    });
  });
  ...
  describe("phoneme", function () {
    ...
    it("should build a phoneme", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><phoneme alphabet='ipa' ph='pi.kæn'>pecan</phoneme></speak>"
      );
    });
    ...
    it("should build a phoneme", function () {
      ...
      assert.equal(
        speech.ssml(),
        "<speak><phoneme alphabet='ipa' ph='pɪ&apos;kɑːn'>pecan</phoneme></speak>"
      );
    });
  });
  ...
  describe("to object", function () {
    ...
    it("should generate a speech tag with a paragraph tag", function () {
      speech.say("Hello");
      speech.paragraph("Mandy");
      assert.deepEqual(speech.toObject(), {
        type: "SSML",
        speech: "<speak>Hello <p>Mandy</p></speak>",
      });
    });
  });
  ...
  describe("emphasis", function () {
    ...
    describe("positive", function () {
      it("should generate the tag with the level and word", function () {
        ...
        assert.equal(
          speech.ssml(),
          "<speak><emphasis level='strong'>really like</emphasis></speak>"
        );
      });
      ...
      it("should generate the tag with the level and word that include special characters", function () {
        ...
        assert.equal(
          speech.ssml(),
          "<speak><emphasis level='strong'>really like &amp; hate</emphasis></speak>"
        );
      });
    });
    ...
    describe("rate", function () {
      describe("positive", function () {
        ...
        it("should generate a prosody tag with the rate attribute and word that include special characters", function () {
          ...
          for (var i = 0; i < validRates.length; i++) {
            ...
            assert.equal(
              ...
              "<speak><prosody rate='" +
                validRates[i] +
                "'>really like &amp; hate</prosody></speak>"
            );
            speech = new Speech();
          }
        });
        ...
        it("should generate a prosody tag with the rate attribute case sensitive", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody rate='x-slow'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the rate attribute being positive %", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody rate='+50%'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the rate attribute", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody rate='40%'>really like</prosody></speak>"
          );
        });
      });
    ...
    describe("pitch", function () {
      describe("positive", function () {
        ...
        it("should generate a prosody tag with the pitch attribute and word case sensitive", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody pitch='x-high'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the pitch attribute positive %", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody pitch='+50%'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the pitch attribute negative %", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody pitch='-40%'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the pitch attribute negative and decimal %", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody pitch='-40.5%'>really like</prosody></speak>"
          );
        });
      });
    ...
    describe("volume", function () {
      describe("positive", function () {
        ...
        it("should generate a prosody tag with the volume attribute and word case sensitive", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody volume='silent'>really like</prosody></speak>"
          );
        });
        ...
        it("should generate a prosody tag with the volume attribute positive dB", function () {
          ...
          assert.equal(
            speech.ssml(),
            "<speak><prosody volume='+6dB'>really like</prosody></speak>"
          );
        });
      });
  ...
  describe("sub", function () {
    ...
    describe("positive", function () {
      ...
      it("should generate a sub tag special characters", function () {
        ...
        assert.equal(
          speech.ssml(),
          "<speak><sub alias='mine'>It&apos;s mine</sub></speak>"
        );
      });
    });
  ...
  describe("sayRandomChoice", function () {
    ...
    describe("positive", function () {
      it("should give you a random item", function () {
        ...
        assert.isTrue(
          count > 1,
          "expected to find more than one element but only got 1"
        );
      });
      ...
      it("should give you a random item", function () {
        ...
        assert.isTrue(
          count === 1,
          "expected to find one element in the results, but got " + count
        );
      });
    });
